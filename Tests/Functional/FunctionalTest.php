...
class FunctionalTest extends JaegerFunctionalTest
{
    ...
    public function testNestedSpansAndHeaderPropagation(string $projectSetup): void
    {
        ...
        $traceAsYAML = $this->getSpansAsYAML($spans, '[].{operationName: operationName, spanID: spanID, references: references, tags: tags[?key==\'http.status_code\' || key==\'command.exit-code\' || key==\'http.url\' || key==\'http.method\'].{key: key, value: value}}');
